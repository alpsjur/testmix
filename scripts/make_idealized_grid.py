...
The file includes the 3D field str_a (structure area density, m⁻¹)
required by the STRUCTURE_MIXING parametrization.
...
-------
    ...
    --input_file Path to a text file specifying xi and eta points where str_a
                 has the specified value. If provided, str_a will be zero
...
def build_str_a(h, pm, pn, z_r, str_a_value, depth_zero_below=None, input_points=None):
    ...
    depth from the sea surface or based on specified points in an input file.
    ...
    input_points   : List of (xi, eta) tuples where str_a should be non-zero.
                     If provided, str_a will be set to 0 elsewhere.
    ...
    if input_points:
        str_a = np.zeros((N_lev, eta_rho, xi_rho), dtype=np.float64)
        for xi, eta in input_points:
            str_a[:, eta, xi] = str_a_value
    else:
...
def read_input_file(file_path):
    """
    Reads a text file containing xi, eta points and returns them as a list of tuples.
    ...
    points : list of tuples
        List of (xi, eta) coordinate pairs.
    """
    points = []
    with open(file_path, 'r') as file:
        for line in file:
            try:
                xi, eta = map(int, line.split())
                points.append((xi, eta))
            except ValueError:
                raise ValueError(f"Invalid line in input file: {line.strip()}")
    return points

def write_grid(output, str_a_value, depth_zero_below, input_file):
    # Load input points if a file is provided
    input_points = read_input_file(input_file) if input_file else None
    ...
    # Build str_a
    str_a = build_str_a(h, pm, pn, z_r, str_a_value, depth_zero_below, input_points)
